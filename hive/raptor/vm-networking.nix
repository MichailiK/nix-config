...
in {
      ...
      builtins.map (vm: {
        ...
        "br-vm${vm.id}" = {
          ...
          NETWORK = {
            ...
            addresses = [{Address = "169.254.${vm.id}.1/32";} {Address = "fe80::1/64";}];
            routes =
  ...
  in {
    netdevs =
      builtins.mapAttrs (
        name: value:
          lib.attrsets.recursiveUpdate value.NETDEV {netdevConfig.Name = name;}
      )
      mergedNetDevices;

    networks = builtins.mapAttrs (name: value:
      lib.attrsets.recursiveUpdate value.NETWORK {matchConfig.Name = name;})
    mergedNetDevices;
  };
      ...
      settings = {
        ...
        */
        ...
        shared-networks = [
          {
            name = "vm-networks";
            relay = {ip-addresses = builtins.map (vm: "169.254.${vm.id}.1") VM_BRIDGES;};
            subnet4 = [
              {
                id = 1;
                subnet = "188.40.162.192/29";
