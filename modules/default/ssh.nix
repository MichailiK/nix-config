...
in
{
      ...
      (pkgs.writeText "mich_ssh_known_hosts" (
        ...
        + lib.concatMapAttrsStringSep "\n\n" (
          ...
          let
            ...
            hostName = if (host.hostName != null) then host.hostName else builtins.concatStringsSep "," host.host;
  ...
  config.programs.ssh.extraConfig =
    let
      ...
      hostsWithOptions = lib.filterAttrs (
        name: value:
          ...
          + lib.concatMapAttrsStringSep "\n\n" (
            ...
            let
              ...
              hostPattern = builtins.concatStringsSep " " (
                host.host ++ (if (host.hostName != null) then [ host.hostName ] else [ ])
              );
              # extraConfig attrset formatted into a (tab-indented) string
              extraConfigFormatted = lib.concatMapAttrsStringSep "\n" (
                name: value:
                "\t${name} ${
                  if value == true then
                    "yes"
            ...
            "# ${name}\n"
            + "Host ${hostPattern}\n"
            + (if (host.hostName != null) then "\tHostname ${host.hostName}\n" else "")
